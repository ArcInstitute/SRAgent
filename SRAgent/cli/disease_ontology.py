...
def disease_ontology_parser(subparsers):
    ...
    sub_parser.add_argument(
        "prompt", type=str, help="Disease description(s) to categorize"
    )
    sub_parser.add_argument(
        "--max-concurrency",
        type=int,
    ...
    sub_parser.add_argument(
        "--recursion-limit", type=int, default=40, help="Maximum recursion limit"
    )
    sub_parser.add_argument(
        "--tenant",
        type=str,
...
def disease_ontology_main(args):
    ...
    results = asyncio.run(
        create_agent_stream(
            input,
            create_disease_ontology_workflow,
            config,
