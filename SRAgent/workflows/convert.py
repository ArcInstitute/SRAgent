...
from typing import Annotated, List, Dict, Sequence, TypedDict, Callable
        ...
        for attempt in range(max_retries):
            try:
                ...
                if (
                    ...
                    print(
                        f"OpenAI refused to extract accessions (attempt {attempt + 1}), retrying...",
                        file=sys.stderr,
                    )
...
def create_router_node() -> Callable:
    ...
    async def invoke_router(
        ...
        accesions = "\n".join([" - SRX: " + format_accessions(state["SRX"]), "\n"])
        ...
        # call the model
        response = await model.with_structured_output(Choice, strict=True).ainvoke(
            formatted_prompt
        )
        # format the response
        return {
...
if __name__ == "__main__":
    ...
    # -- nodes --#
    state = {"entrez_id": "34748561", "messages": []}
