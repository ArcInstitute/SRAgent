...
def test_google_search():
    ...
    search_terms = [
        ...
        "SRP009262",  # Different SRA accession
        "PRJNA192983",  # BioProject accession
        "GSE63525",   # GEO accession
        "\"Hi-C method\"",  # Scientific method in quotes
    ...
    for term in search_terms:
        ...
        try:
            ...
            if isinstance(results, str) and results.startswith("ERROR"):
                ...
                # If no results, try to get more information
                if len(results) == 0:
                    print("No results found. Trying to get more information...")
                    from googleapiclient.discovery import build
                    service = build("customsearch", "v1", developerKey=api_key)
                    res = service.cse().list(q=term, cx=cse_id, num=2).execute()
                    print(f"Total results according to API: {res['searchInformation']['totalResults']}")
                    print(f"Search time: {res['searchInformation']['searchTime']} seconds")
                    print(f"Formatted total results: {res['searchInformation']['formattedTotalResults']}")
...
def test_google_search_tool():
    """Test the Google search tool function"""
    print("\n=== Testing google_search_tool function ===")
    try:
        # For LangChain tools, we need to use the invoke method
        # or call it directly without keyword arguments
        results = google_search_tool("GSE63525")
        print(results)
    except Exception as e:
        print(f"Error testing google_search_tool: {e}")
